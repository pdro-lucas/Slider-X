...
function saveFileInDatabase(oldImageName, oldImageLocation, req, res) {
  let { title, tag, description, priority, active, showInfo } = req.body;
  ...
  const sql = `UPDATE image SET title=?, tag=?, description=?, priority=?, image_name=?, image_location=?, active=?, show_info=? WHERE id = ${req.params.id}`;
  connect
