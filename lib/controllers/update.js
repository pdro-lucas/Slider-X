...
function saveFileInDatabase(oldImageName, oldImageLocation, req, res) {
  ...
  if (showInfo === "on") showInfo = 1;
  if (!transitionTime) transitionTime = 8;
  ...
  const sql = `UPDATE image SET title=?, tag=?, description=?, priority=?, transition_time=?, image_name=?, image_location=?, active=?, show_info=? WHERE id = ${req.params.id}`;
  connect
